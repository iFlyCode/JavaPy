...
public class LineSegment {
	 ...
	 * </p>
	 * <p>
	 * Gets the y-intercept of the line segment
	 * </p>
	 * 
			...
			if (Math.max(seg.x1, seg.x2) >= Math.min(this.x1, this.x2)) {
				if (Math.max(this.y1, this.y2) >= Math.min(seg.y1, seg.y2)) {
					if (Math.max(seg.y1, seg.y2) >= Math.min(this.y1, this.y2)) {
						boolean PartA = ((this.x2 - this.x1)
								* (seg.y1 - this.y1) - (this.y2 - this.y1)
								* (seg.x1 - this.x1)) > 0;
						boolean PartB = ((this.x2 - this.x1)
								* (seg.y2 - this.y1) - (this.y2 - this.y1)
								* (seg.x2 - this.x1)) < 0;

						boolean PartC = ((seg.x2 - this.x1)
								* (this.y1 - seg.y1) - (this.y2 - seg.y1)
								* (this.x1 - seg.x1)) > 0;
						boolean PartD = ((seg.x2 - this.x1)
								* (this.y2 - seg.y1) - (this.y2 - seg.y1)
								* (this.x2 - seg.x1)) < 0;
	...
	/**
	 * <ul>
	 * <li><b><i> intersectionOf </i></b></li>
	 * </ul>
	 * <p style="font-family:Courier">
	 * public double intersectionOf(LineSegment seg)
	 * </p>
	 * <p>
	 * Returns the intersection point of two line segments, or returns null if they don't intersect.
	 * </p>
	 * 
	 * @param seg
	 *            - The line segment to test for intersection.
	...
	public double[] intersectionOf(LineSegment seg) {
		if (!this.intersects(seg))
			return null;
		double x = (seg.yIntercept() - this.yIntercept())
				/ (this.slope() - seg.slope());
